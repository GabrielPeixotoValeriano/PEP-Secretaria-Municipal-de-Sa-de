document.addEventListener('DOMContentLoaded', function() {
    ...
    function calculateAge(dob) {
        ...
        // Ajusta os meses se o dia atual for menor que o dia do nascimento
        if (now.getDate() < dob.getDate()) {
            months--;
        }

        // Se os meses estiverem negativos, ajusta os anos e corrige os meses
        if (months < 0) {
            years--;
            months += 12;
        }
    ...
    document.getElementById('patient-form').addEventListener('submit', function(event) {
        ...
        // Calcula a idade em anos e meses utilizando a função calculateAge
        const age = calculateAge(dob);
        
        const healthUnit = document.getElementById('health-unit').value;
        const patientName = document.getElementById('patient-name').value;
        const patientSocialName = document.getElementById('patient-social-name').value; // Novo campo opcional
        ...
        prescriptionHtml += 
            `<div class="patient-info">
                ...
                <p><strong>NOME SOCIAL:</strong> ${patientSocialName ? patientSocialName.toUpperCase() : "-"}</p>
                <p>
        ...
        if (treatmentMessage !== "") {
            ...
            if (treatmentMessage.toUpperCase().includes("PEP RN")) {
                prescriptionHtml += `<p><a href="https://azt.aids.gov.br/documentos/siclom_operacional/Solicita%C3%A7%C3%A3o_Medicamentos_Profilaxia_Preven%C3%A7%C3%A3o_transmiss%C3%A3o_vertical_OUTUBRO_2024_DIGIT%C3%81VEL.pdf" target="_blank" style="color: blue; text-decoration: underline;">FORMULÁRIO DE DISPENSAÇÃO DE ARV - PROFILAXIA - TRANSMISSÃO VERTICAL HIV</a></p>`;
            }
            ...
            if (resultDTG5mg) {
                prescriptionHtml += `<li><strong>1 - ${resultDTG5mg.medication.toUpperCase()}</strong>, tomar <strong>${resultDTG5mg.comprimidos}</strong> comprimidos de 5mg diluídos em 5 a 10 ml de água, uma vez ao dia, durante 28 dias.</li>`;
